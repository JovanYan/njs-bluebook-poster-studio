...
- **页面背景（最终定稿，2026-07-21）**：回到最初版本——白/暖白底 + `body::before` 双层 `radial-gradient` 钴蓝点阵（22px 间距错位排列，`::after` 径向渐隐到边缘）+ 3 个荧光笔模糊椭圆斑点（黄 `rgba(255,230,0,…)` ×2、粉 `rgba(255,92,168,…)` ×1，26px 模糊、奇数角度旋转、`pointer-events:none`），移动端斑点透明度 0.7
  - **背景演进记录**（均已被用户否决并移除，勿恢复）：①单台卡通 TV → ②极简 TV + 书脊字编辑拼贴 → ③四屏电视墙 + ticker → ④巨型网点渐变墙 + 兔子轮廓。结论：用户最终选择「简单点阵」为定稿；`public/assets/tv/` 海报 PNG 保留未用
...
*文档版本：v1.2 · 2026-07-21 · 覆盖功能：半调引擎、闪光三调系统、25+9 贴纸、名牌生成器、四组贴纸栏、移动端适配、GitHub API 备份；背景定稿为初版点阵+荧光斑点（TV 系列方案已否决移除）*
